...
SOURCE_FILE = 'source_data/nrg_d_hhq_linear.csv'
...
def transform_fill_standardised_values(measurement: Measurement) -> Measurement:
    # transformation implements the following
    # based on the assumption that the standard unit is always TJ (terajoules)
    # if the measurement unit is already TJ then set the standardised values to the raw values
    # if not then we do no transformation
    if measurement.measurent_unit == 'TJ':
...
def transform_merge_with_existing_measurement(measurement: Measurement, db_cursor) -> Measurement:
    ...
    db_cursor.execute('''
              SELECT * FROM measurement where
                ...
                country_code = ? and
                year = ?
                ''', (measurement.seic_code, measurement.nrg_bal_code, measurement.country_code, measurement.year)
    )
...
def load_record(measurement: Measurement, db_cursor: Any, db_conn: Any) -> bool:
    ...
    db_cursor.execute('''
              ...
              VALUES (?, ?, ?, ?, ?, ?, ?, ?)
              ''', (measurement.seic_code, measurement.nrg_bal_code, measurement.country_code, measurement.year, measurement.measurement_value, measurement.measurent_unit, measurement.standardised_measurement_value, measurement.standardised_measurement_unit)
    )
    db_conn.commit()
    return True
...
if __name__ == '__main__':
    db_conn = sqlite3.connect('datastore.db')
    db_cursor = db_conn.cursor()

    counter = 0
    for record in data_generator(SOURCE_FILE):
        print(counter)
        counter += 1
